...
get isSubmitDisabled() {
    ...
    formatActionReviews(actionResults) {
        return (actionResults || []).map(ar => {
            // Initialize field extraction variables
            ...
            // Extract fields from the fields array returned by Apex
            if (ar.fields && Array.isArray(ar.fields)) {
                ar.fields.forEach(field => {
                    const { apiName, label, value } = field;
                    
                    if (!value) return; // Skip empty values
                    
                    // Extract subject (handles both Subject and opportunity Name fields)
                    if ((apiName === 'Subject' || label === 'Subject') || 
                        (apiName === 'Name' || label === 'Opportunity Name')) {
                        if (apiName === 'Name' || label === 'Opportunity Name') {
                            extractedFields.opportunity = value;
                        } else {
                            extractedFields.subject = value;
                        }
                    }
                    
                    // Extract task-specific fields populated by Agentforce
                    switch (apiName) {
                        case 'ActivityDate':
                            extractedFields.activityDate = value;
                            break;
                        case 'OwnerId':
                            extractedFields.ownerId = value;
                            break;
                        case 'WhatId':
                            extractedFields.whatId = value;
                            break;
                    }
                });
            }
            
            // Determine card title priority: Opportunity Name > Subject > Default
            const cardTitle = extractedFields.opportunity || extractedFields.subject || 'Follow-Up Action';

            // Prepare display values with fallbacks for empty fields
            const taskFieldDisplays = {
                displaySubject: extractedFields.subject || 'No subject',
                displayDueDate: extractedFields.activityDate ? this.formatDate(extractedFields.activityDate) : 'No due date',
                displayAssignedTo: extractedFields.ownerId || 'Not assigned',
                displayRelatedTo: extractedFields.whatId || 'Not related'
            };

            // Determine submit button label based on object type
            const submitButtonLabel = this.getSubmitButtonLabel(ar.action?.AF_Object_Name__c);

            // Filter out fields already displayed in task details section
            const displayedFieldNames = ['Subject', 'ActivityDate', 'OwnerId', 'WhatId'];
            const otherFields = (ar.fields || []).filter(field =>
                !displayedFieldNames.includes(field.apiName) && 
                !(field.apiName === 'Name' && field.label === 'Opportunity Name')
            );
