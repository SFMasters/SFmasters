...
get isSubmitDisabled() {
    ...
    async processActionData(result) {
        if (result && result.length > 0) {
            // Use lead's decorateFieldsForDisplay pattern first
            const decoratedResults = await Promise.all((result || []).map(async ar => ({
                ...ar,
                fields: await this.decorateFieldsForDisplay(ar.fields)
            })));
            
            this.actionReviews = this.formatActionReviews(decoratedResults);
    ...
    // Enhanced version of final org's formatActionReviews method with our field display functionality
    formatActionReviews(actionResults) {
        return (actionResults || []).map(ar => {
            ...
            let cardTitle = 'Follow-Up Action'; // Default title
            ...
            // NEW: Add field display properties using decorated fields (.uiValue)
            const fieldMap = {};
            ar.fields.forEach(f => {
                fieldMap[f.apiName] = f;
            });

            const isTaskObject = apiName === 'task';
            ...
            if (isTaskObject) {
                taskProps = {
                    hasTaskFields: true,
                    subjectValue: fieldMap['Subject']?.uiValue || 'No subject',
                    dueDateValue: fieldMap['ActivityDate']?.uiValue || 'No due date',
                    assignedToValue: fieldMap['OwnerId']?.uiValue || 'Not assigned',
                    relatedToValue: fieldMap['WhatId']?.uiValue || 'Not related'
                };
            } else if (isOpportunityObject) {
                oppProps = {
                    hasOpportunityFields: true,
                    nameValue: fieldMap['Name']?.uiValue || 'No name',
                    amountValue: fieldMap['Amount']?.uiValue || 'No amount',
                    clientNameValue: fieldMap['AF_Client_Name__c']?.uiValue || 'No client'
                    // productGroupValue: fieldMap['AF_Product_group__c']?.uiValue || 'No product group' // For final org
                };
            }
